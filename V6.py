# V5.py — IAN‑R1 (TRIGA) en OpenMC con patrón del núcleo idéntico a la figura:
# - Núcleo en bloque con muesca (L-shape)
# - 7 canales de irradiación alineados en la fila inferior
# - Barras de control y de aire en las posiciones de la figura
# - Combustible definido sólo en peso (wo) para evitar errores
# - Fuente moderna (IndependentSource)
#
# Ejecuta:
#   python3 V5.py
#   # (corre la simulación y genera geometry.xml/materials.xml/settings.xml/tallies.xml)
...
# ================= Lattice idéntico a la figura =================
# 9x9 (x=columnas, y=filas). Fila 8 (última) = 7 canales de irradiación (blancos).
# 'F'=combustible, 'C'=control, 'A'=aire, 'O'=canal irradiación, 'W'=agua/reflector.
lattice_map = [
#   c0  c1  c2  c3  c4  c5  c6  c7  c8   (fila 0 = arriba)
  ['W','W','W','W','W','W','W','W','W'],  # espacio reflector superior
  ['W','F','F','F','F','F','W','W','W'],  # muesca derecha superior
  ['W','F','F','F','F','F','F','W','W'],
  ['W','F','C','F','F','F','F','F','W'],
  ['W','F','F','F','F','F','F','F','W'],
  ['W','W','F','F','F','F','F','F','W'],  # muesca izquierda inferior
  ['W','W','F','F','A','F','F','F','W'],  # barra de aire interior
  ['W','W','F','F','F','F','C','F','W'],  # barra de control interior
  ['W','O','O','O','O','O','O','O','W']   # 7 canales de irradiación alineados
]
...
# Caja interna del núcleo (exacta al tamaño del lattice)
...
# Caja externa del reflector (un poco mayor)
...
block_region_3d = refl_box_region  # insertamos el bloque dentro de la piscina
